    ...
    const fetchContact = async () => {
      try {
        ...
        if (response.ok) {
          ...
          setErrors({ submit: "Failed to fetch contact details" })
        }
      } catch (error) {
        console.error("Error fetching contact:", error)
        setErrors({ submit: "An error occurred while fetching the contact" })
      } finally {
        setLoading(false)
      }
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const data = await response.json()
        setErrors({ submit: data.message || "Failed to update contact" })
      }
    } catch (error) {
      console.error("Error updating contact:", error)
      setErrors({ submit: "An error occurred while updating the contact" })
    }
  }
    ...
    <AppLayout title="Edit Contact">
      ...
      <Modal isOpen={showSuccessModal} onClose={handleSuccessModalClose}>
        <div className="text-center">
          <p className="mb-4">Your contact has been updated successfully!</p>
